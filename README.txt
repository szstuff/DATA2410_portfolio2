README for a file transfer program with custom-made error control methods created for an exam for course DATA2410 at OsloMet
Created by:	STILIAN TODOROV ZAGOROV – S364531
		NGOC LINN VU HUYNH – S364523
		SALONI ARORA CHITKARA – S364538
		JOVIA NAMAYANJA KAGGWA – S364526
Last updated: 15/05/2023

The application is a file transfer program created in Python that uses UDP with custom-made error-handling code to transfer files over the network. The program transfers data reliably and can handle issues related to packets received out of order, duplicate packets, and missing packets.  
...
Python3 required
header.py required to run the application (packed with the application)
...
Output: 
The output from the application is printed in the terminal. The client prints information like the measured RTT, transfer time and measured throughput. The client and server both print some configuration info like the protocol, file, testcase and window size used for the transfer. 

The code is designed to be able to run with default values (except for file path, which must always be provided). However issues might occur when only -s or -c argument are specified. If issues occur when running, try running the script with all values specified. 